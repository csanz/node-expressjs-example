 ...
 * and our core middleware functions.
 *
 * - logger
 * - session
 */
...
function setTheme (app) {
  ...
  app.set('themeDirectory', themeDirectory)

  // Set jade as view engine
  app.set('view engine', 'jade')

  // Style middleware settings
  // We need to `wrap it` though
  ...
  function stylusMiddlewareWrapper (req, res, next) {
    if (req.url.match(/^\/admin\//)) {
      // We are not compiling admin's css with stylus
      return next()
    } else {
  ...
  // Now, we add it into the middleware stack
  app.use(stylusMiddlewareWrapper)
